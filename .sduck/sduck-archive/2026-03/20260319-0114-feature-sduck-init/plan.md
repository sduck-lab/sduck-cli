...
- `.sduck/sduck-assets/` 아래 기본 생성 대상 파일 목록을 코드 상수로 정의한다.
...
- `.sduck/sduck-assets/`와 `.sduck/sduck-workspace/`가 없으면 생성한다.
...
- `.sduck/sduck-workspace/`는 디렉토리 보장만 수행하고 내부 작업 디렉토리는 건드리지 않는다.
- 사용자 커스텀 파일을 덮어쓸 수 있는 대상은 `.sduck/sduck-assets/`의 기본 파일로 한정한다.
...
- 저장소의 기준 asset 원본은 현재 프로젝트 루트 `.sduck/sduck-assets/`로 본다.
...
- 최소 아래 시나리오를 검증한다.
  - 신규 초기화 시 `.sduck/sduck-assets/`, `.sduck/sduck-workspace/` 생성
