...
`sduck init`를 interactive 모드로 실행할 때 선택 UI의 조작 방식이 충분히 안내되지 않아서, 사용자가 아무 agent도 선택하지 않은 채 제출하거나 기대와 다른 방식으로 입력할 수 있다. 그 결과 선택된 agent에 맞는 root markdown 파일이나 `.sduck/sduck-assets/agent-rules` 기반 규칙 내용이 생성되지 않아 init이 실패한 것처럼 보인다.
