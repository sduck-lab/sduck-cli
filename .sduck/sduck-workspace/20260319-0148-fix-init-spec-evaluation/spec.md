...
현재 `sduck init`은 `.sduck/sduck-assets/plan-evaluation.yml`과 스펙 템플릿 파일들은 생성하지만,
워크플로우에서 실제로 사용하는 `.sduck/sduck-assets/spec-evaluation.yml`은 기본 생성 대상에 포함하지 않는다.
...
2. 생성된 `.sduck/sduck-assets/` 내용을 확인한다.
...
| 방안 B | `.sduck/sduck-assets/`를 동적으로 스캔해 전부 생성 | 누락 가능성 감소               | 현재 init 정책과 테스트 변경 범위가 커짐 |
...
target_paths:
  ...
  - .sduck/sduck-workspace/20260319-0148-fix-init-spec-evaluation/spec.md
```
...
- [x] `sduck init` 실행 후 `.sduck/sduck-assets/spec-evaluation.yml` 누락을 검증하는 테스트를 추가한다
...
- 사용자가 수동으로 `.sduck/sduck-assets/spec-evaluation.yml`을 복사해 둘 수 있다.
...
- `.sduck/sduck-assets/spec-evaluation.yml`
- `src/core/init.ts`
