...
현재 `sduck init`은 `.sduck/sduck-assets/`와 `.sduck/sduck-workspace/`를 초기화할 수 있지만,
...
- 공통 규칙에는 최소 아래가 포함되어야 한다
  - 세션 시작 시 `.sduck/sduck-workspace/`와 현재 상태 확인
  - 승인 전 코드 작성 금지
  - `spec.md`, `plan.md`, `meta.yml` 상태 전이 규칙
  - `<-` 사용자 메모 규칙
  - `.sduck/sduck-assets/spec-evaluation.yml`, `.sduck/sduck-assets/plan-evaluation.yml` 사용 규칙
...
| Assets   | `.sduck/sduck-assets/agent-rules/*` 또는 동등 구조 | 공통 규칙/에이전트별 템플릿 저장              |
