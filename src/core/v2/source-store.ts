...
export function loadSourceBundle(projectRoot: string): SourceBundle {
  ...
  validateUnique(
    bundle.contextItems.map((item) => item.id),
    'contextItems.id',
  );
  validateUnique(
    bundle.briefSnapshots.map((snapshot) => snapshot.id),
    'briefSnapshots.id',
  );
  validateUnique(
    bundle.implementationTraces.map((trace) => trace.id),
    'implementationTraces.id',
  );
  validateUnique(
...
function assertUnmappedDecision(
  ...
  if (value['files'] !== undefined) assertArray(value['files'], filePath, `${field}.files`);
  if (value['appliesTo'] !== undefined)
    assertArray(value['appliesTo'], filePath, `${field}.appliesTo`);
}
