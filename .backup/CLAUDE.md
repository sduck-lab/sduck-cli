...
- Start each session by checking `.sduck/sduck-workspace/` and any active task `meta.yml`.
...
- Claude는 문서를 읽을 때 본문과 함께 이 메모도 반드시 반영한다
...
SPEC_APPROVED           → (Claude)      → PENDING_PLAN_APPROVAL
PENDING_PLAN_APPROVAL   → (사용자 승인) → IN_PROGRESS
IN_PROGRESS             → (Claude)      → DONE
```

| 상태                    | Claude 가능 행동          | Claude 불가 행동     |
| ----------------------- | ------------------------- | -------------------- |
