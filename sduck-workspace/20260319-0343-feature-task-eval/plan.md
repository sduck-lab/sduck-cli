...
- `sduck-assets/eval/task.yml`을 추가한다.
...
- 기본 흐름을 아래처럼 명시한다.
  ...
  - `task eval`
  - `task done`
- task 완료 직전에는 `sduck-assets/eval/task.yml`을 읽고 그 기준으로 평가하도록 적는다.

## Step 4. agent rule 자산에도 task eval 경로 반영

- `sduck-assets/agent-rules/core.md`에 task eval 경로와 사용 시점을 추가한다.
...
- `AGENT.md`, `CLAUDE.md`, `sduck-assets/agent-rules/core.md`의 경로와 흐름 문구가 일치하는지 확인한다.
