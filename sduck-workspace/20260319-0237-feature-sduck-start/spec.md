...
- 타입별 `spec.md` 템플릿이 `sduck-assets/` 기준으로 정확히 적용된다
...
- [x] AC1: `sduck start <type> <slug>` 실행 시 `sduck-workspace/YYYYMMDD-HHmm-{type}-{slug}/` 디렉토리가 생성된다
...
- [x] AC5: `spec.md`는 `sduck-assets/`의 타입별 템플릿을 사용해 생성된다
...
- 현재 지원 type은 최소 아래를 포함한다
  ...
  - `chore`
  - `build`
- `type`은 `sduck-assets/types/{type}.md` 템플릿과 매핑된다
...
```text
✅ 작업 디렉토리 생성됨
   경로: sduck-workspace/YYYYMMDD-HHmm-{type}-{slug}/
...
- [ ] `build` 타입은 `sduck-assets/types/build.md` 템플릿을 사용한다
