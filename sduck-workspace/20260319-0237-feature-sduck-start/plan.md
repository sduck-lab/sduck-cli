...
- 이 작업에서는 asset 디렉토리 구조도 함께 정리한다.
  - 스펙 템플릿은 `sduck-assets/types/{type}.md` 기준으로 이동한다
  - 평가 기준은 `sduck-assets/eval/spec.yml`, `sduck-assets/eval/plan.yml` 기준으로 이동한다
...
- `sduck-workspace/` 아래 `IN_PROGRESS`, `PENDING_SPEC_APPROVAL`, `PENDING_PLAN_APPROVAL` 상태의 작업을 탐지한다.
...
- `type`별로 `sduck-assets/types/{type}.md`를 매핑한다.
- `build` 타입은 `sduck-assets/types/build.md`를 사용한다.
...
- 기존 asset 경로를 새 구조로 정리한다.
  - `sduck-assets/spec-*.md` → `sduck-assets/types/*.md`
  - `sduck-assets/spec-evaluation.yml` → `sduck-assets/eval/spec.yml`
  - `sduck-assets/plan-evaluation.yml` → `sduck-assets/eval/plan.yml`
- `init`, 규칙 문서, 평가 로직, 테스트에서 참조하는 경로를 모두 새 구조로 맞춘다.
- 새 구조에서 누락 자산이 없도록 `init` 기본 생성 대상도 함께 갱신한다.

## Step 7. workspace 디렉토리와 초기 파일 생성 구현

- `sduck-workspace/`가 없으면 생성한다.
...
- 최소 아래 시나리오를 검증한다.
  - feature 작업 생성
  - fix 작업 생성
  - build 작업 생성 시 `sduck-assets/types/build.md` 사용
  - 잘못된 type 실패
  - 잘못된 slug 실패
  - 활성 작업 존재 시 차단
